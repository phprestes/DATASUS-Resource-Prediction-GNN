...
anterior e escreve na seguinte, sem nunca escrever para trás.
    ...
    01_raw           ZIP de competência baixados do CNES (src/extract.py)
    02_intermediate  um DuckDB por competência, tudo VARCHAR (src/to_sql.py)
    03_primary       Parquet por competência e tabela, tipado (src/to_parquet.py)
    04_feature       eventos de mudança entre competências (src/changes.py)
"""
...
# Eventos de mudança materializados por src/changes.py.
CHANGES_FOLDER = FEATURE_FOLDER / "changes"

DOCS_DIR = BASE_DIR / "docs"

# Fonte da verdade do schema, lida por src/schema.py.

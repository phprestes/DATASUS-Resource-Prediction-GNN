...
- **[docs/03-decisoes.md](docs/03-decisoes.md)** — 44 numbered decisions with the evidence behind each and what was rejected. When something in the code looks surprising, the reason is usually a D-nn entry.
...
- **The bar to beat is MAP@10 = 0.2725** — and it is held by `popularidade_item`, not by a GNN. Persistence returns AP exactly equal to prevalence and AUC exactly 0.500 — if it ever doesn't, the harness is broken (D-24).
- **Latest results** (D-44, test 2026, paired on 127.9k positionable establishments): `gnn_relacional` AP **0.00650** / AUC **0.841** / MAP@10 0.2533; `gnn_geografica` 0.00493 / 0.800 / 0.2665; `por_entidade` 0.00335 / 0.737 / 0.158; `gbdt_geral` 0.00289 / 0.751 / 0.191; `popularidade_item` 0.00220 / 0.699 / **0.2725**; `persistencia` 0.00055 / 0.500 / 0.033.
- **The two headline metrics disagree, and that disagreement is the finding.** The relational GNN wins AP (2.3× the best structure-free model) and AUC, and **loses MAP@10** to a model that ignores the establishment entirely. Structure tells you *where* in the network an acquisition happens, not *which* equipment a given unit buys. Never report one metric without the other beside it.
